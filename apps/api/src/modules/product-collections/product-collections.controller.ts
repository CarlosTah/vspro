...
export class ProductCollectionsController {
  ...
  @Post()
  @Roles('admin', 'manager')
  create(@Body() dto: CreateCollectionDto, @TenantSchema() schema: string) {
    return this.collections.create(dto, schema);
  }

  @Get()
  @Roles('admin', 'manager', 'operator')
  findAll(@TenantSchema() schema: string) {
    return this.collections.findAll(schema);
  }

  @Get(':id')
  @Roles('admin', 'manager', 'operator')
  findOne(@Param('id', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.collections.findById(id, schema);
  }
  ...
  update(
    ...
    return this.collections.update(id, dto, schema);
  }

  @Delete(':id')
  @Roles('admin')
  remove(@Param('id', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.collections.delete(id, schema);
  }

  @Get('for-product/:productId')
  @Roles('admin', 'manager', 'operator')
  forProduct(@Param('productId', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.collections.getRecommendationsForProduct(id, schema);
  }
}
