...
export class MaintenanceTicketsService {
  ...
  async assign(ticketId: string, providerId: string, schemaName: string) {
    ...
    const provider = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT name, phone FROM "${schemaName}".service_providers WHERE id = $1::uuid`,
      providerId,
    );
    if (provider[0]?.phone) {
  ...
  async authorize(ticketId: string, schemaName: string) {
    await this.prisma.$executeRawUnsafe(
      `UPDATE "${schemaName}".maintenance_tickets SET status = 'authorized', authorized_at = NOW() WHERE id = $1::uuid`,
      ticketId,
    );
    return this.findById(ticketId, schemaName);
  }
  ...
  async complete(ticketId: string, schemaName: string) {
    await this.prisma.$executeRawUnsafe(
      `UPDATE "${schemaName}".maintenance_tickets SET status = 'completed', completed_at = NOW() WHERE id = $1::uuid`,
      ticketId,
    );
    return this.findById(ticketId, schemaName);
  }
