...
const PROPERTIES = [
  {
    ...
    description:
      'Estudio moderno de 35m² en el centro histórico. Cocina equipada, WiFi de alta velocidad, Smart TV. Ideal para viajeros de negocios.',
  ...
  },
  {
    ...
    description:
      'Departamento de 55m² en la Roma Norte. 1 recámara con cama king, sala-comedor, balcón con vista a parque. Zona de restaurantes y cafés.',
    price: 1800,
    category: '1 Recámara',
    images: [
  ...
  },
  {
    ...
    description:
      'Amplio departamento de 80m² en la Condesa. 2 recámaras (king + twin), 2 baños completos, terraza privada con asador. Pet-friendly.',
    price: 2800,
  ...
  },
  {
    ...
    description:
      'Penthouse de lujo de 120m² en Polanco. 2 recámaras master con baño completo, sala de estar, comedor para 8, terraza panorámica con jacuzzi.',
    price: 5500,
    category: 'Penthouses',
    images: [
  ...
  },
  {
    ...
    description:
      'Loft de 65m² con techos altos y diseño industrial. Espacio abierto con mezzanine, cocina tipo bar, ventanales de piso a techo.',
    price: 1600,
    category: 'Lofts',
    images: [
  ...
  },
  {
    ...
    description:
      'Casa completa de 150m² en Coyoacán. 3 recámaras, jardín privado, zona de juegos infantiles. Cerca de museos y mercados artesanales.',
    price: 3200,
    category: 'Casas',
    images: [
...
async function main() {
  ...
  for (const p of PROPERTIES) {
    ...
    await prisma.$executeRawUnsafe(
      ...
      id,
      JSON.stringify(p.blockingDates),
    );
  }
  ...
  for (let i = 0; i < reservations.length; i++) {
    ...
    if (r.status === 'payment_verified') {
      await prisma.$executeRawUnsafe(
        ...
        orderId,
        totalPrice,
        `DEP-R359-${Math.random().toString(36).substring(2, 8).toUpperCase()}`,
      );
    }
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const msgs = [
      ...
      {
        dir: 'outbound',
        text: '¡Hola! 🏠 Soy Luna de Room 359. Con gusto te ayudo. ¿Para cuántas personas sería y cuántas noches necesitas?',
      },
      { dir: 'inbound', text: 'Somos 2 personas, del viernes al domingo' },
      {
        dir: 'outbound',
        text: 'Perfecto, 2 noches para 2 personas. Te recomiendo nuestro Studio Ejecutivo Centro a $1,200/noche o el Loft Industrial Juárez a $1,600/noche. Ambos incluyen WiFi, cocina y Smart TV. ¿Cuál te interesa más?',
      },
    ];
  ...
  for (const p of PROPERTIES) {
    console.log(
      `   ${p.name}: $${p.rates.perNight}/noche | $${p.rates.perWeek}/semana | $${p.rates.perMonth}/mes`,
    );
  }
  console.log('');
}
