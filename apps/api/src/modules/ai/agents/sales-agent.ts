...
IMPORTANTE: Si no puedes resolver algo (soporte técnico, devoluciones), indica que transferirás a un compañero.`;
    ...
    return [
      {
        ...
        function: {
          name: 'show_catalog',
          description:
            'Muestra productos del catálogo con imágenes al cliente. Usa cuando preguntan "qué tienen", "muéstrame vestidos", o piden ver opciones.',
          parameters: {
            type: 'object',
            properties: {
              ...
              category: {
                type: 'string',
                description: 'Categoría específica (Vestidos, Chamarras, etc.)',
              },
              limit: {
                type: 'number',
                description: 'Máximo de productos a mostrar (default 5, max 10)',
              },
            },
          },
        },
      },
      {
        ...
        function: {
          ...
          description:
            'Muestra detalle completo de un producto con variantes (tallas, colores). Usa cuando el cliente pregunta por un producto específico.',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          name: 'suggest_upsell',
          description:
            'Sugiere productos complementarios o de mayor valor. Usa DESPUÉS de agregar al carrito o al mostrar detalle de producto. No lo uses en cada mensaje.',
          parameters: {
            type: 'object',
            properties: {
              productId: {
                type: 'string',
                description: 'ID del producto base (del que se quiere upsell)',
              },
              productName: { type: 'string', description: 'Nombre del producto base' },
              category: { type: 'string', description: 'Categoría actual' },
              strategy: {
                type: 'string',
                enum: ['upsell', 'cross_sell', 'complement', 'auto'],
                description: 'Estrategia (default: auto)',
              },
            },
          },
        },
      ...
      },
      {
        ...
        function: {
          name: 'add_to_cart',
          description:
            'Agrega un producto al carrito del cliente. Usa cuando dice "quiero", "agrégame", "me llevo".',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          name: 'show_cart',
          description:
            'Muestra el carrito actual del cliente. Usa cuando pregunta "qué llevo", "mi carrito", "cuánto es".',
      ...
      },
      {
        ...
        function: {
          name: 'confirm_order',
          description:
            'Confirma el pedido y convierte el carrito en una orden real. Usa cuando el cliente dice "es todo", "confirmo", "listo".',
          parameters: { type: 'object', properties: {} },
        },
      },
      {
        ...
        function: {
          ...
          description:
            'Envía datos bancarios para transferencia al cliente. Usa DESPUÉS de confirmar pedido.',
          parameters: {
            type: 'object',
            properties: {
              orderId: {
                type: 'string',
                description: 'ID del pedido (se obtiene de confirm_order)',
              },
            },
            required: ['orderId'],
          },
      ...
      },
      {
        ...
        function: {
          ...
          description:
            'Verifica un comprobante de pago (foto). Usa cuando el cliente envía una imagen después de solicitar el pago.',
          parameters: {
            type: 'object',
            properties: {
    ...
    switch (name) {
      case 'show_catalog': {
        ...
        if (products.length === 0)
          return JSON.stringify({
            found: false,
            message: 'No encontré productos con esa búsqueda',
          });
        ...
        return JSON.stringify({
          ...
          instruction:
            'Presenta estos productos al cliente. Si tienen imageUrl, menciona que puedes enviar la foto. Usa emojis y formato amigable.',
        });
      }
      ...
      case 'show_product_detail': {
        ...
        return JSON.stringify({
          ...
          instruction:
            'Muestra el detalle del producto. Si tiene variantes, pregunta cuál prefiere. Ofrece agregarlo al pedido.',
        });
      }
      ...
      case 'apply_discount': {
        ...
        `,
          newTotal,
          `\n[Descuento ${args.discountPercent}%: -$${discount.toFixed(2)} — ${args.reason}]`,
          args.orderId,
        );
      ...
      case 'suggest_upsell': {
        ...
        const complements: Record<string, string[]> = {
          Vestidos: ['Accesorios', 'Calzado'],
          Conjuntos: ['Accesorios', 'Calzado'],
          Pantalones: ['Playeras', 'Chamarras'],
        ...
        if (args.strategy === 'upsell' && category) {
          ...
          const basePrice = productId
            ? ((
                await this.prisma.$queryRawUnsafe<any[]>(
                  `SELECT price FROM "${schemaName}".products WHERE id = $1::uuid`,
                  productId,
                )
              )[0]?.price ?? 0)
            : 0;
      ...
      case 'create_order': {
        ...
        return JSON.stringify({
          success: true,
          message: 'Pedido creado (delegado a OrdersService)',
        });
      }
      ...
      case 'add_to_cart': {
        ...
        if (!prods[0])
          return JSON.stringify({ success: false, message: `No encontré "${args.productName}"` });

        const p = prods[0];
        const qty = args.quantity ?? 1;
        if ((p.stock_available ?? 0) < qty)
          return JSON.stringify({
            success: false,
            message: `Solo hay ${p.stock_available} disponibles`,
          });
        ...
        return JSON.stringify({
          ...
          message: `✅ Agregado: ${qty}x ${p.name} ($${(price * qty).toLocaleString()})`,
        });
      }
      ...
      case 'show_cart': {
        ...
        if (cart.items.length === 0)
          return JSON.stringify({
            empty: true,
            message: 'Carrito vacío. ¿Qué te gustaría agregar?',
          });
        const summary = cart.items.map(
          (i: any) =>
            `${i.quantity}x ${i.productName} — $${(i.unitPrice * i.quantity).toLocaleString()}`,
        );
        return JSON.stringify({
          items: summary,
          total: cart.total,
      ...
      case 'remove_from_cart': {
        ...
        const idx = cart.items.findIndex((i: any) =>
          i.productName.toLowerCase().includes(args.productName.toLowerCase()),
        );
        if (idx < 0)
          return JSON.stringify({
            success: false,
            message: `"${args.productName}" no está en el carrito`,
          });
      ...
      case 'confirm_order': {
        const cart = await this.getCartFromContext(context.conversationId, schemaName);
        if (cart.items.length === 0)
          return JSON.stringify({ success: false, message: 'El carrito está vacío' });
        if (!context.customerId)
          return JSON.stringify({ success: false, message: 'No pude identificar al cliente' });
        ...
        for (const item of cart.items) {
          await this.prisma.$executeRawUnsafe(
            `UPDATE "${schemaName}".inventory SET stock_available = stock_available - $1, stock_reserved = stock_reserved + $1 WHERE product_id = $2::uuid AND stock_available >= $1`,
            item.quantity,
            item.productId,
          );
        }
        ...
        return JSON.stringify({
          ...
          total: cart.total,
          itemCount: cart.itemCount,
          message: `🎉 ¡Pedido creado! ${orders[0].order_number} — Total: $${cart.total.toLocaleString()}`,
          orderId: orders[0].id,
        });
      }

      case 'request_payment': {
        const ord = await this.prisma.$queryRawUnsafe<any[]>(
          `SELECT order_number, total, status FROM "${schemaName}".orders WHERE id = $1::uuid`,
          args.orderId,
        );
        ...
        if (ord[0].status !== 'new')
          return JSON.stringify({
            success: false,
            message: `Pedido ya en estado "${ord[0].status}"`,
          });

        const aiCfg = await this.prisma.$queryRawUnsafe<any[]>(
          `SELECT agent_config->'payment_info' AS info FROM "${schemaName}".ai_config LIMIT 1`,
        );
        const info = aiCfg[0]?.info ?? {
          bank: 'BBVA',
          clabe: '012180001234567890',
          beneficiary: 'Demo SA de CV',
        };

        await this.prisma.$executeRawUnsafe(
          `UPDATE "${schemaName}".orders SET status = 'payment_pending', updated_at = NOW() WHERE id = $1::uuid`,
          args.orderId,
        );
        await this.prisma.$executeRawUnsafe(
          `INSERT INTO "${schemaName}".payments (order_id, method, amount, status, reference) VALUES ($1::uuid, 'transfer', $2, 'pending', $3)`,
          args.orderId,
          parseFloat(ord[0].total),
          `REF-${ord[0].order_number}`,
        );
      ...
      case 'verify_payment_image': {
        const ordR = await this.prisma.$queryRawUnsafe<any[]>(
          `SELECT o.order_number, o.total, p.id AS pid FROM "${schemaName}".orders o LEFT JOIN "${schemaName}".payments p ON p.order_id = o.id AND p.status = 'pending' WHERE o.id = $1::uuid`,
          args.orderId,
        );
        ...
        return JSON.stringify({
          verified: false,
          message: `📷 Comprobante recibido para ${ordR[0].order_number} ($${parseFloat(ordR[0].total).toLocaleString()}). Verificando...`,
          needsReview: true,
        });
      }
  ...
  private async saveCartToContext(
