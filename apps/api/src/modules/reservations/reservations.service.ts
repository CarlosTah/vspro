...
export class ReservationsService {
  ...
  async create(dto: CreateReservationDto, schema: string) {
    ...
    if (checkOut <= checkIn)
      throw new BadRequestException('Check-out debe ser después de check-in');
    ...
    if (conflicts.length > 0) {
      throw new BadRequestException(
        `Fechas no disponibles. Hay ${conflicts.length} reserva(s) en ese rango.`,
      );
    }
    ...
    const totalPrice =
      dto.totalPrice ??
      (await this.calculatePrice(dto.checkIn, dto.checkOut, schema, dto.propertyId));
  ...
  async delete(id: string, schema: string) {
    await this.prisma.$executeRawUnsafe(
      `DELETE FROM "${schema}".reservations WHERE id = $1::uuid`,
      id,
    );
    return { success: true };
  }
  ...
  async createPricingRule(dto: PricingRuleDto, schema: string) {
    ...
    await this.prisma.$executeRawUnsafe(
      `ALTER TABLE "${schema}".pricing_rules ADD COLUMN IF NOT EXISTS price_per_week DECIMAL(10,2)`,
    );
    await this.prisma.$executeRawUnsafe(
      `ALTER TABLE "${schema}".pricing_rules ADD COLUMN IF NOT EXISTS price_per_month DECIMAL(10,2)`,
    );
    ...
    ); // is_default if no dates specified
  ...
  async deletePricingRule(id: string, schema: string) {
    await this.prisma.$executeRawUnsafe(
      `DELETE FROM "${schema}".pricing_rules WHERE id = $1::uuid`,
      id,
    );
    return { success: true };
  }
  ...
  async calculatePrice(
    ...
    while (current < end) {
      ...
      const seasonalRule = rules.find(
        (r) =>
          !r.isDefault && r.dateFrom && r.dateTo && dateStr >= r.dateFrom && dateStr <= r.dateTo,
      );
