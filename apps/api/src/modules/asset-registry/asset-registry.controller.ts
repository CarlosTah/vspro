...
export class AssetRegistryController {
  ...
  create(@Body() dto: CreateAssetDto, @TenantSchema() schema: string) {
    return this.assets.create(dto, schema);
  }

  @Get('customer/:customerId')
  @Roles('admin', 'manager', 'operator')
  getByCustomer(@Param('customerId', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.assets.getByCustomer(id, schema);
  }

  @Get(':id')
  @Roles('admin', 'manager', 'operator')
  findOne(@Param('id', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.assets.findById(id, schema);
  }
  ...
  update(
    ...
    return this.assets.update(id, body.details, schema);
  }

  @Get(':id/history')
  @Roles('admin', 'manager', 'operator')
  history(@Param('id', ParseUUIDPipe) id: string, @TenantSchema() schema: string) {
    return this.assets.getServiceHistory(id, schema);
  }
}
