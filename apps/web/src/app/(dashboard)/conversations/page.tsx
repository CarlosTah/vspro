...
export default function ConversationsPage() {
  ...
  useEffect(() => {
    ...
    const fetchMessages = () => {
      api
        .get<any[]>(`/conversations/${selectedId}/messages?limit=100`)
        .then((newMsgs) => {
  ...
  useEffect(() => {
    ...
    api
      .get<any[]>(`/delivery/drivers/${selectedDriverId}/messages`)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      api
        .get<any[]>(`/delivery/drivers/${selectedDriverId}/messages`)
        .then((msgs) => {
          if (msgs && msgs.length !== driverChatMessages.length) setDriverChatMessages(msgs);
        })
        .catch(() => {});
    }, 3000);
                ...
                <div className="flex items-center justify-between border-b border-gray-700 px-5 py-3">
                  <div>
                    <p className="font-medium text-white">
                      {drivers?.find((d: any) => d.id === selectedDriverId)?.name ?? 'Repartidor'}
                    </p>
                    <p className="text-xs text-gray-400">
                      WhatsApp · {drivers?.find((d: any) => d.id === selectedDriverId)?.phone ?? ''}
                    </p>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-3">
                  {loadingDriverMsgs ? (
                    ...
                    <p className="text-center text-gray-500 text-sm py-8">
                      Sin mensajes con este repartidor
                    </p>
                  ) : (
            ...
            {!selectedId ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between border-b border-gray-700 px-5 py-3">
                  <div>
                    <p className="font-medium text-white">
                      {selectedConv?.customerName ?? 'Cliente'}
                    </p>
                    <p className="text-xs text-gray-400">
                      {selectedConv?.channelType ?? 'whatsapp'} ·{' '}
                            ...
                            <div className="flex items-center gap-1 mt-1.5 pt-1.5 border-t border-gray-700/50">
                              {msg.rating === 'up' ? (
                                ...
                                <>
                                  <button
                                    onClick={async () => {
                                      await api.patch(`/conversations/messages/${msg.id}/rate`, {
                                        rating: 'up',
                                      });
                                      setMessages(
                                        messages.map((m: any) =>
                                          m.id === msg.id ? { ...m, rating: 'up' } : m,
                                        ),
                                      );
                                    }}
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={async () => {
                                    ...
                                    await api.post(`/conversations/messages/${msg.id}/correct`, {
                                      correction: correctionText,
                                    });
                                    setMessages(
                                      messages.map((m: any) =>
                                        m.id === msg.id
                                          ? { ...m, rating: 'down', correction: correctionText }
                                          : m,
                                      ),
                                    );
                                    setCorrecting(null);
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const res = await fetch(
                            `${API_URL}/conversations/${selectedId}/send-media`,
                            {
                              method: 'POST',
                              headers: {
