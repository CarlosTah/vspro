...
export class MediaAssetsController {
 ...
 `,
    ...
    if (
      (body.type === 'menu' || body.type === 'catalog' || body.type === 'promo') &&
  ...
  @Roles('admin', 'manager')
  async createFromUrl(
    @Body() body: { type: string; title: string; url: string },
    @TenantSchema() schema: string,
  ) {
    await this.ensureTable(schema);
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
 ...
 `,
  ...
  async delete(
    ...
    if (deleteProducts === 'true') {
      ...
      await this.prisma.$executeRawUnsafe(
        `DELETE FROM "${schema}".media_assets WHERE id = $1::uuid`,
        id,
      );
      return {
        success: true,
        productsDeleted: count[0]?.cnt ?? 0,
        message: 'Material y productos eliminados',
      };
    }
  ...
  private async extractProductsFromMenu(
    buffer: Buffer,
    mimeType: string,
    schema: string,
  ): Promise<void> {
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Eres un extractor de menús. Analiza la imagen y extrae TODOS los productos con precios. Responde SOLO con un JSON array: [{"name":"string","price":number,"category":"string"}]. Si no ves precio, usa 0. SOLO JSON, nada más.',
          },
          {
            role: 'user',
            content: [
      ...
      if (added > 0)
        console.log(`[MediaAssets] Auto-extracted ${added} products from menu for ${schema}`);
    } catch (err: any) {
  ...
  private async uploadToSpaces(file: any, schema: string): Promise<string> {
    try {
      ...
      const spacesEndpoint =
        this.config.get('AWS_S3_ENDPOINT') ||
        this.config.get('DO_SPACES_ENDPOINT') ||
        'https://nyc3.digitaloceanspaces.com';
      const spacesBucket =
        this.config.get('AWS_S3_BUCKET') || this.config.get('DO_SPACES_BUCKET') || 'vspro-uploads';
      const spacesKey =
        this.config.get('AWS_ACCESS_KEY_ID') || this.config.get('DO_SPACES_KEY') || '';
      const spacesSecret =
        this.config.get('AWS_SECRET_ACCESS_KEY') || this.config.get('DO_SPACES_SECRET') || '';
