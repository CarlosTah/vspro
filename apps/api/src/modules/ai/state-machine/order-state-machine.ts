...
export class OrderStateMachine {
  ...
  private handleIdle(state: ConversationStateData, intent: ParsedIntent): TransitionResult {
    switch (intent.type) {
      ...
      case 'want_to_order': {
        ...
        const hasRealName =
          state.customerName && !state.customerName.includes('~') && state.customerName.length > 2;
      ...
      case 'add_items':
        ...
        if (validatedIdle.valid.length > 0) {
          ...
          const invalidNote =
            validatedIdle.invalid.length > 0
          ...
          actions: [
            {
              ...
              args: { orderNumber: intent.orderNumber ?? state.orderNumber },
            },
          ],
          llmContext:
            'Informa al cliente el estado de su pedido basándote en el resultado de la herramienta.',
        };
      ...
      case 'check_menu': {
        ...
        return {
          newState: OrderState.TAKING_ORDER,
          actions: [
            { tool: 'send_media_to_customer', args: { mediaType: isPromo ? 'promo' : 'menu' } },
          ],
          llmContext: isPromo
      ...
      case 'complaint':
        return {
          ...
          actions: [
            { tool: 'escalate_complaint', args: { reason: intent.text, priority: 'medium' } },
          ],
          llmContext:
            'El cliente tiene una queja. Sé empático, discúlpate y confirma que ya se escaló al equipo.',
        };
      ...
      case 'cancel':
        if (state.orderId) {
          return {
            newState: OrderState.IDLE,
            actions: [
              { tool: 'cancel_order', args: { orderId: state.orderId, reason: intent.text } },
            ],
        ...
        return {
          ...
          actions: [],
          llmContext:
            'El cliente quiere cancelar pero no hay pedido activo. Pregunta en qué puedes ayudar.',
        };
      ...
      default: {
        ...
        if (timeWords.some((k) => intent.text.toLowerCase().includes(k))) {
          return {
            ...
            actions: [],
            llmContext:
              'El cliente pregunta cuánto tarda. Responde: "Los pedidos para recoger tardan aprox. 15-25 minutos. Para domicilio, 30-45 minutos dependiendo la zona."',
          };
        }
    ...
    switch (intent.type) {
      ...
      case 'check_menu': {
        ...
        return {
          newState: OrderState.TAKING_ORDER,
          actions: [
            { tool: 'send_media_to_customer', args: { mediaType: wantsPromo ? 'promo' : 'menu' } },
          ],
  ...
  private handleConfirming(state: ConversationStateData, intent: ParsedIntent): TransitionResult {
    switch (intent.type) {
      case 'confirm_yes': {
        ...
        const mentionsCod = ['efectivo', 'contra entrega', 'al repartidor', 'cash'].some((k) =>
          txt.includes(k),
        );
        const mentionsPickup = ['paso', 'recojo', 'recoger', 'recogo', 'paso a recoger'].some((k) =>
          txt.includes(k),
        );
      ...
      default:
        ...
        if (
          intent.type === 'other' ||
          intent.type === 'want_to_order' ||
          intent.type === 'check_menu'
        ) {
    ...
    switch (intent.type) {
      ...
      case 'want_pickup':
        return {
          ...
          actions: [],
          llmContext:
            'El cliente recoge en local. Pregunta forma de pago: "¿Pagas por transferencia o en efectivo?"',
        };
              ...
              actions: [
                {
                  ...
                  args: { orderId: state.orderId, items: validatedAdd.valid },
                },
              ],
              llmContext: '',
              skipLlm: true,
  ...
  private handleSettingAddress(
    ...
    switch (intent.type) {
      ...
      default: {
        ...
        const isQuestion =
          text.includes('?') ||
          text.startsWith('por q') ||
          text.startsWith('por que') ||
          text.startsWith('porq') ||
        ...
        return {
          ...
          actions: [],
          llmContext:
            'Necesito la dirección completa. Pide: calle, número, colonia, referencias. También que mande su ubicación 📍.',
        };
      }
    }
  }

  private handleAskingPayment(
    state: ConversationStateData,
    intent: ParsedIntent,
  ): TransitionResult {
    switch (intent.type) {
      case 'want_cod':
        return {
          newState: OrderState.ORDER_COMPLETE,
          actions: [
            { tool: 'set_payment_method', args: { orderId: state.orderId, method: 'cod' } },
          ],
          llmContext: '',
          skipLlm: true,
      ...
      case 'want_transfer':
        return {
          ...
          llmContext:
            'Solicita el pago por transferencia. Da los datos bancarios si los tienes y pide que envíe el comprobante. NO incluyas listas de productos ni inventes información.',
        };
      ...
      default: {
        ...
        if (transferWords.some((k) => intent.text.toLowerCase().includes(k))) {
          return {
            ...
            actions: [{ tool: 'request_payment', args: { orderId: state.orderId } }],
            llmContext:
              'Solicita el pago por transferencia. Da los datos bancarios y pide comprobante.',
          };
        }
        return {
          newState: OrderState.ASKING_PAYMENT,
  ...
  private handleProcessingPayment(
    state: ConversationStateData,
    intent: ParsedIntent,
  ): TransitionResult {
    if (intent.type === 'payment_proof') {
      return {
        ...
        actions: [],
        llmContext:
          'El cliente envió comprobante de pago. Confirma: "Recibimos tu comprobante, lo estamos verificando. Tu pedido fue enviado a cocina. 🙌"',
      };
    }
    return {
      newState: OrderState.PROCESSING_PAYMENT,
      actions: [],
      llmContext:
        'Estamos esperando el comprobante de pago. Recuérdale que envíe foto de la transferencia.',
    };
  }
  ...
  private handleOrderComplete(
    ...
    if (intent.type === 'want_to_order' || intent.type === 'add_items') {
      return this.handleIdle(
        { ...state, state: OrderState.IDLE, items: undefined, orderId: undefined },
        intent,
      );
    }
    if (intent.type === 'complaint') {
      return {
        ...
        llmContext:
          'El cliente tiene un problema con su pedido entregado. Sé empático, discúlpate, y confirma que se escaló al equipo para resolverlo.',
      };
    }
    ...
    if (timeQ.some((k) => intent.text.toLowerCase().includes(k))) {
      return {
        ...
        actions: [],
        llmContext:
          'El cliente pregunta cuánto tarda. Responde que su pedido ya está en preparación/camino según el estado. Si es pickup: "15-25 min". Si delivery: "30-45 min aprox."',
      };
    }
    ...
    if (complaintWords.some((k) => intent.text.toLowerCase().includes(k))) {
      return {
        ...
        llmContext:
          'El cliente reporta un problema con su pedido. Sé empático y confirma que se escaló al equipo.',
      };
    }
  ...
  private validateItems(items: Array<{ productName: string; quantity: number; notes?: string }>): {
    ...
    for (const item of items) {
      ...
      if (!match) {
        match = this.catalog.find(
          (p) => p.name.toLowerCase() === inputName || inputName === p.name.toLowerCase(),
        );
      }
      ...
      if (!match) {
        ...
        for (const [category, keywords] of Object.entries(typePatterns)) {
          if (keywords.some((kw) => inputName.includes(kw))) {
            ...
            const categoryProducts = this.catalog.filter(
              (p) =>
            ...
            const variant = inputName
              .replace(
                /^(sope|torta|taco|gringa|burrito|costra|nacho|nachos|huarache|papa rellena)s?\s*(de\s*)?/i,
                '',
              )
              .trim();
            ...
            if (variant) {
              ...
              if (sizeMatch && category === 'Bebidas') {
                ...
                match = categoryProducts.find(
                  (p) =>
                    p.name.toLowerCase().includes(variant.replace(sizeMatch[0], '').trim()) &&
                ...
                if (!match) {
                  ...
                  match = categoryProducts.find(
                    (p) =>
                      p.name.includes(size) &&
                      (variant.includes('coca') ? p.name.toLowerCase().includes('coca') : true),
                  );
                }
              }
              if (!match) {
      ...
      if (!match) {
        ...
        const normalized = inputName
          .replace(/^(tacos|sopes|tortas|gringas|burritos|costras|nachos|huaraches)/i, (m) =>
            m.slice(0, -1),
          ) // plural → singular (remove trailing 's')
      ...
      if (!match) {
        ...
        for (const product of this.catalog) {
          ...
          if (
            score > bestScore ||
            (score === bestScore && bestMatch && product.price < bestMatch.price)
          ) {
            bestScore = score;
            bestMatch = product;
          }
  ...
  private calculateTotal(
    ...
    return items.reduce((sum, item) => {
      const price =
        item.price ??
        this.catalog.find((p) => p.name.toLowerCase() === item.productName.toLowerCase())?.price ??
        0;
  ...
  private formatOrderSummary(
    items: Array<{ productName: string; quantity: number; notes?: string; price?: number }>,
  ): string {
    return items
      .map((i) => {
        const price =
          i.price ??
          this.catalog.find((p) => p.name.toLowerCase() === i.productName.toLowerCase())?.price ??
          0;
