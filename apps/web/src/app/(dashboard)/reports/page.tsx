...
export default function ReportsPage() {
  ...
  const { data: report, loading } = useApi<any>(
    `/reports/summary?from=${range.from}&to=${range.to}`,
  );
        ...
        {period === 'custom' && (
          <div className="flex gap-2 ml-2">
            <input
              ...
              className="vspro-input text-sm w-40"
            />
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              icon="💰"
              label="Revenue"
              value={`$${Number(revenue.total ?? 0).toLocaleString('es-MX')}`}
              sub="MXN"
            ...
            <StatCard
              ...
              sub={`Entregados: ${orders.delivered ?? 0}`}
              color="text-blue-400"
            />
            <StatCard
              icon="🎟️"
              label="Ticket promedio"
              value={`$${orders.total > 0 ? Math.round((revenue.total ?? 0) / orders.total).toLocaleString('es-MX') : '0'}`}
              sub="por pedido"
            ...
            <StatCard
              icon="👥"
              label="Clientes"
              value={customers.newInPeriod ?? customers.total ?? 0}
              sub="en el período"
              color="text-cyan-400"
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-green-400">
                    ${Number(revenue.paid ?? 0).toLocaleString('es-MX')}
                  </span>
                </div>
                <div className="w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${revenue.total > 0 ? ((revenue.paid ?? 0) / revenue.total) * 100 : 0}%`,
                    }}
                  />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-yellow-400">
                    ${Number(revenue.pending ?? 0).toLocaleString('es-MX')}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-card-border bg-card p-5">
              ...
              <div className="space-y-2">
                ...
                {orders.total === 0 && (
                  <p className="text-sm text-gray-500">Sin pedidos en este período</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-400">
                  {report?.conversations?.resolved ?? 0}
                </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-gray-700 bg-gray-800 p-4">
          <p className="text-2xl font-bold text-red-300">
            ${metrics.overall.lostRevenue.toLocaleString('es-MX')}
          </p>
