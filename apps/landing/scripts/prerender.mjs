      ...
      traverse(ast, {
        ...
        VariableDeclarator(path) {
          if (
            ...
            !isUppercaseName(path.node.id.name) ||
            !path.node.init
          ) {
            return;
          }
        ...
        CallExpression(path) {
          ...
          if (
            !component ||
            (!t.isFunctionExpression(component) && !t.isArrowFunctionExpression(component))
          ) {
            return;
          }
          ...
          const declaratorName =
            declarator?.isVariableDeclarator() && t.isIdentifier(declarator.node.id)
...
async function run() {
  ...
  try {
    ...
    const { default: App } = await import(
      `${resolve(cwd, 'dist-ssr/App.js')}?prerender=${Date.now()}`
    );
    ...
    try {
      if (window?.happyDOM) await window.happyDOM.close();
    } catch {}
    try {
      rmSync(resolve(cwd, 'dist-ssr'), { recursive: true, force: true });
    } catch {}
  }
}
