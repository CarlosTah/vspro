    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <MetricCard
          ...
          value={`$${income.grossRevenue.toLocaleString()}`}
          color="text-green-400"
        />
        <MetricCard
          label="Ingresos netos"
          value={`$${income.netRevenue.toLocaleString()}`}
          color="text-emerald-400"
        />
        <MetricCard
          label="IVA recaudado"
          value={`$${income.taxCollected.toLocaleString()}`}
          color="text-blue-400"
        />
        <MetricCard
          label="Envíos"
          value={`$${income.shippingRevenue.toLocaleString()}`}
          color="text-cyan-400"
        />
        <MetricCard
          label="Reembolsos"
          value={`-$${income.refunds.toLocaleString()}`}
          color="text-red-400"
        />
      </div>
        ...
        <div className="rounded-xl border border-gray-700 bg-gray-800 p-5">
          ...
          <div className="space-y-3">
            <PaymentRow
              ...
              value={accounting.totalEntries}
              color="text-white"
            />
            <PaymentRow
              label="Ventas"
              value={`$${accounting.sales.toLocaleString()}`}
              color="text-green-400"
            />
            <PaymentRow
              label="Envíos"
              value={`$${accounting.shipping.toLocaleString()}`}
              color="text-cyan-400"
            />
            <PaymentRow
              label="Reembolsos"
              value={`-$${accounting.refunds.toLocaleString()}`}
              color="text-red-400"
            />
            <PaymentRow
              label="Ajustes"
              value={`$${accounting.adjustments.toLocaleString()}`}
